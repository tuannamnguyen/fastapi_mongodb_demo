...
@student_router.get("", dependencies=[Depends(JWTBearer())], status_code=status.HTTP_200_OK)
async def get_all_students() -> list[dict]:
    return [bson_to_dict(student) async for student in db.students.find()]


@student_router.post("", dependencies=[Depends(JWTBearer())], status_code=status.HTTP_201_CREATED)
...
@student_router.get("/{student_id}", dependencies=[Depends(JWTBearer())], status_code=status.HTTP_200_OK)
...
@student_router.put("/{student_id}", dependencies=[Depends(JWTBearer())], status_code=status.HTTP_201_CREATED)
...
@student_router.delete("/{student_id}", dependencies=[Depends(JWTBearer())], status_code=status.HTTP_200_OK)
